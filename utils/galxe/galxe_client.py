...
from .galxe_utils import make_x_unique_link_id, generate_ga_cookie_value
...
class GalxeClient():
    ...
    async def claim_points(self, campaign_id: str):
        ...
        prepare = await self.prepare_participate(campaign_id=campaign_id,chain=chain,capthca=captcha, mint_count=params['mintCount'], point_mint_amount=params['pointMintAmount'])
        ...
        claim = await self.galxe_onchain.handle_claim_onchain_points(loyalty_point_address=loyalty_point_address,verify_ids=verify_ids,amounts=amounts, claim_fee=claim_fee,signature=signature)
        return claim
