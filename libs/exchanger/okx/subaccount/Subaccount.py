...
class Subaccount(Base):
    ...
    async def list(
        ...
        response = await self.make_request(method=Methods.GET, request_path=f"/api/v5/users/{self.section}/{method}", body=request_params(body))
        subaccounts = {}
    ...
    async def asset_balances(self, subAcct: str, token_symbol: str | None = None) -> dict[str, FundingToken]:
        ...
        response = await self.make_request(method=Methods.GET, request_path=f"/api/v5/asset/{self.section}/{method}", body=request_params(body))
        tokens = {}
        for token in response.get("data"):
